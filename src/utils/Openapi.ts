...
function isDeRefResponse(defObject: object): defObject is DeRefResponse<unknown> {
  return !!(defObject && (defObject as DeRefResponse<unknown>)?.deRefData?.path);
}
...
const recursiveDeRefInternal = (refOrObject: object, context: RecursiveDeRefContext): object => {
  ...
  if (isAReference(refOrObject)) {
    if (context.entities[refOrObject.$ref]) {
      ...
      if (isDeRefResponse(deRefObj) && deRefObj.deRefData) {
        context.entities[deRefObj.deRefData.path] = deRefObj;
      }
    }
  } else {
    deRefObj = {
